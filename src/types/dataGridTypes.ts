...
export type SortConfig = { key: "id" | null; direction: "asc" | "desc" };
...
export type BodyProps<T extends { id: string | number }> = {
  row: T;
  rows: T[];
  columns: ColumnDefinition<T>[];
  isChecked: boolean;
...
export type HeaderProps<T extends { id: string | number }> = {
  columns: ColumnDefinition<T>[];
